        ...
        <div className="space-y-3 sm:space-y-4">
          {faqs.map((faq, i) => (
            <div key={i} className="group bg-white rounded-lg sm:rounded-xl border border-gray-100 hover:border-primary/30 px-4 sm:px-6 shadow-sm hover:shadow-lg transition-all duration-300">
              <button className="flex flex-1 items-center justify-between gap-3 w-full text-left text-sm sm:text-base font-semibold py-4 sm:py-5 cursor-pointer group-hover:text-primary transition-colors"
                onClick={() => setOpen(open === i ? null : i)}>
                <span className="flex items-center gap-2 sm:gap-3">
                  <span className="w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-primary/10 flex items-center justify-center flex-shrink-0 group-hover:bg-primary/20 transition-colors">
                    <i className="ri-question-line text-primary text-sm sm:text-base"></i>
                  </span>
                  <span>{faq.q}</span>
                </span>
              ...
              {open === i && (
                <div className="pb-4 sm:pb-5 text-sm sm:text-base text-muted-foreground leading-relaxed">{faq.a}</div>
              )}
            </div>
          ))}
        </div>
