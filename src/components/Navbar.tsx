...
export default function Navbar() {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 bg-white/90 backdrop-blur-lg shadow-sm`}>
          ...
          <nav className="hidden md:flex items-center gap-6 lg:gap-8">
            {['Beneficios', 'Funcionalidades', 'Precio', 'FAQ'].map((item) => (
              <a key={item} href={`#${item.toLowerCase()}`}
                className="text-muted-foreground hover:text-foreground transition-colors font-medium text-sm lg:text-base relative after:absolute after:bottom-0 after:left-0 after:w-0 after:h-0.5 after:bg-primary after:transition-all hover:after:w-full">
                {item}
              </a>
            ))}
          </nav>
          <button className="md:hidden p-2 hover:bg-accent rounded-lg transition-all duration-300 cursor-pointer active:scale-95"
